...
int MbdMonDraw::MakeCanvas(const std::string &name)
{
  ...
  if (name == "MbdMon1")
  {
    ...
    TC[0] = new TCanvas(name.c_str(), "MbdMon Example Monitor", -1, 0, xsize / 2, ysize);
    // root is pathetic, whenever a new TCanvas is created root piles up
    // 6kb worth of X11 events which need to be cleared with
    // gSystem->ProcessEvents(), otherwise your process will grow and
    // grow and grow but will not show a definitely lost memory leak
    gSystem->ProcessEvents();
    Pad[0] = new TPad("mbdpad1", "who needs this?", 0.1, 0.5, 0.9, 0.9, 0);
    Pad[1] = new TPad("mbdpad2", "who needs this?", 0.1, 0.05, 0.9, 0.45, 0);
    Pad[0]->Draw();
    Pad[1]->Draw();
  ...
  else if (name == "MbdMon2")
  {
    ...
    TC[1] = new TCanvas(name.c_str(), "MbdMon2 Example Monitor", -xsize / 2, 0, xsize / 2, ysize);
    gSystem->ProcessEvents();
    Pad[2] = new TPad("mbdpad3", "who needs this?", 0.1, 0.5, 0.9, 0.9, 0);
    Pad[3] = new TPad("mbdpad4", "who needs this?", 0.1, 0.05, 0.9, 0.45, 0);
    Pad[2]->Draw();
    Pad[3]->Draw();
    // this one is used to plot the run number on the canvas
  ...
  {
    TC[2] = new TCanvas(name.c_str(), "MbdMon3 Example Monitor", xsize / 2, 0, xsize / 2, ysize);
    gSystem->ProcessEvents();
    Pad[4] = new TPad("mbdpad5", "who needs this?", 0.1, 0.5, 0.9, 0.9, 0);
    Pad[5] = new TPad("mbdpad6", "who needs this?", 0.1, 0.05, 0.9, 0.45, 0);
    Pad[4]->Draw();
    Pad[5]->Draw();
    // this one is used to plot the run number on the canvas
...
int MbdMonDraw::DrawFirst(const std::string & /* what */)
{
  OnlMonClient *cl = OnlMonClient::instance();
  TH1 *mbdmon_hist1 = cl->getHisto("mbdmon_hist1");
  TH1 *mbdmon_hist2 = cl->getHisto("mbdmon_hist1");
...
int MbdMonDraw::DrawSecond(const std::string & /* what */)
{
  OnlMonClient *cl = OnlMonClient::instance();
  TH1 *mbdmon_hist1 = cl->getHisto("mbdmon_hist2");
  TH1 *mbdmon_hist2 = cl->getHisto("mbdmon_hist2");
...
int MbdMonDraw::DrawDeadServer(TPad *transparentpad)
{
  transparentpad->cd();
  TText FatalMsg;
  ...
  FatalMsg.DrawText(0.5, 0.9, "MBD MONITOR");
