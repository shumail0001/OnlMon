...
OnlBbcEvent::OnlBbcEvent(const int cal_pass) :
  _tres(0.05),
  _calpass(cal_pass)
{
  // set default values
...
int OnlBbcEvent::SetRawData(Event *event, MbdPmtContainer *bbcpmts)
{
  // First check if there is any event (ie, reading from PRDF)
  if (event == nullptr || bbcpmts == nullptr)
  {
    return BbcMonDefs::DISCARDEVENT;
  }

  int evt_type = event->getEvtType();
  if (evt_type != DATAEVENT)
  {
    std::cout << PHWHERE << "OnlBbcEvent: Event type is not DATAEVENT, skipping" << std::endl;
    return BbcMonDefs::DISCARDEVENT;
  }
...
///
int OnlBbcEvent::Calculate(MbdPmtContainer *bbcpmts, MbdOut *bbcout)
{
  if ( _debugintt )
  {
    _verbose = 100;
  ...
  if (_verbose >= 10)
  {
    std::cout << "In OnlBbcEvent::Calculate() " << m_evt << std::endl;
  }
  Clear();
  if (bbcout != nullptr)
  {
...
int OnlBbcEvent::DoQuickClockOffsetCalib()
{
...
int OnlBbcEvent::FillSampMaxCalib()
{
...
int OnlBbcEvent::Read_Charge_Calib(const std::string &gainfname)
{
...
int OnlBbcEvent::Read_TQ_T0_Offsets(const std::string &t0cal_fname)
{
...
int OnlBbcEvent::Read_TQ_CLK_Offsets(const std::string &t0cal_fname)
{
  std::ifstream tcalibfile(t0cal_fname);
...
int OnlBbcEvent::Read_TT_CLK_Offsets(const std::string &t0cal_fname)
{
  std::ifstream tcalibfile(t0cal_fname);
...
void OnlBbcEvent::ReadSyncFile(const char *fname)
{
  Int_t f_evt{0};
  UShort_t f_femclk{0};
  Float_t f_bz{0.};
