...
int OnlBbcSig::FillSplineTemplate()
{
  ...
  if (max < template_min_good_amplitude)
  {
    return 0;
  }
  ...
  // cout << f_ampl << endl;
  if (f_ampl < template_min_good_amplitude || f_ampl > template_max_good_amplitude)
  {
    return 0;
  }
...
void OnlBbcSig::MakeAndWriteTemplate(ostream& out, ostream& oerr)
{
  ...
  if (verbose)
  {
    cout << "In  OnlBbcSig::MakeAndWriteTemplate" << endl;
  }

  // Int_t nbinsy = h2Template->GetNbinsX();

  TString name = h2Template->GetName();
  name += "_py";
  ...
  for (int ibin = 0; ibin < template_npointsx; ibin++)
  {
    ...
    */
    ...
    if (verbose)
    {
      cout << "ibin " << ibin << "\t" << template_y[ibin] << "\t" << template_yrms[ibin] << endl;
    }
    // delete hprojy;
  }
  delete hprojy;
...
void OnlBbcSig::FillFcnTemplate()
{
  ...
  if (verbose)
  {
    cout << "ampl time " << ch << "\t" << f_ampl << "\t" << f_time << endl;
  }
  hAmpl->Fill(f_ampl);
  hTime->Fill(f_time);
...
int OnlBbcSig::ReadTemplate(ifstream& shapefile, ifstream& sherrfile)
{
  ...
  while (shapefile >> temp_ch >> temp_nsamples >> temp_begintime >> temp_endtime)
  {
    if (verbose)
    {
      cout << "shape " << temp_ch << "\t" << temp_nsamples << "\t" << temp_begintime << "\t" << temp_endtime << endl;
    }
    if (temp_ch != ch)
    {
  ...
  while (sherrfile >> temp_ch >> temp_nsamples >> temp_begintime >> temp_endtime)
  {
    if (verbose)
    {
      cout << "sherr " << temp_ch << "\t" << temp_nsamples << "\t" << temp_begintime << "\t" << temp_endtime << endl;
    }
    if (temp_ch != ch)
    {
