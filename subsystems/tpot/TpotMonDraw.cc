...
int TpotMonDraw::MakeCanvas(const std::string &name)
{
  ...
  if (name == "TpotMon1")
  {
    ...
    TC[0] = new TCanvas(name.c_str(), "TpotMon Example Monitor", -1, 0, xsize / 2, ysize);
    // root is pathetic, whenever a new TCanvas is created root piles up
    // 6kb worth of X11 events which need to be cleared with
    // gSystem->ProcessEvents(), otherwise your process will grow and
    // grow and grow but will not show a definitely lost memory leak
    gSystem->ProcessEvents();
    Pad[0] = new TPad("tpotpad1", "who needs this?", 0.1, 0.5, 0.9, 0.9, 0);
    Pad[1] = new TPad("tpotpad2", "who needs this?", 0.1, 0.05, 0.9, 0.45, 0);
    Pad[0]->Draw();
    Pad[1]->Draw();
  ...
  else if (name == "TpotMon2")
  {
    // xpos negative: do not draw menu bar
    TC[1] = new TCanvas(name.c_str(), "TpotMon2 Example Monitor", -xsize / 2, 0, xsize / 2, ysize);
    gSystem->ProcessEvents();
    Pad[2] = new TPad("tpotpad3", "who needs this?", 0.1, 0.5, 0.9, 0.9, 0);
    Pad[3] = new TPad("tpotpad4", "who needs this?", 0.1, 0.05, 0.9, 0.45, 0);
    Pad[2]->Draw();
    Pad[3]->Draw();
    // this one is used to plot the run number on the canvas
  ...
  else if (name == "TpotMon3")
  {
    TC[2] = new TCanvas(name.c_str(), "TpotMon3 Example Monitor", xsize / 2, 0, xsize / 2, ysize);
    gSystem->ProcessEvents();
    Pad[4] = new TPad("tpotpad5", "who needs this?", 0.1, 0.5, 0.9, 0.9, 0);
    Pad[5] = new TPad("tpotpad6", "who needs this?", 0.1, 0.05, 0.9, 0.45, 0);
    Pad[4]->Draw();
    Pad[5]->Draw();
    // this one is used to plot the run number on the canvas
...
int TpotMonDraw::DrawFirst(const std::string & /* what */)
{
  OnlMonClient *cl = OnlMonClient::instance();
  TH1 *tpotmon_hist1 = cl->getHisto("tpotmon_hist1");
  TH1 *tpotmon_hist2 = cl->getHisto("tpotmon_hist1");
  if (!gROOT->FindObject("TpotMon1"))
  {
    MakeCanvas("TpotMon1");
  }
...
int TpotMonDraw::DrawSecond(const std::string & /* what */)
{
  OnlMonClient *cl = OnlMonClient::instance();
  TH1 *tpotmon_hist1 = cl->getHisto("tpotmon_hist2");
  TH1 *tpotmon_hist2 = cl->getHisto("tpotmon_hist2");
  if (!gROOT->FindObject("TpotMon2"))
  {
    MakeCanvas("TpotMon2");
  }
...
int TpotMonDraw::DrawHistory(const std::string & /* what */)
{
  int iret = 0;
  ...
  std::string varname = "tpotmondummy";
