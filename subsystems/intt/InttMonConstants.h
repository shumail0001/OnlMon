...
namespace INTT
{
	...
	constexpr int LAYER_OFFSET = 3;
	constexpr int LADDER[LAYER] = {12, 12, 16, 16};
	constexpr int NORTHSOUTH = 2;
	...
	constexpr int CHIPS =  CHIP * NORTHSOUTH * LADDERS;
	...
	namespace HitMap
	{
		int FindGlobalBin(int& bin, int layer, int ladder, int northsouth, int chip, int channel)
		{
			...
			if(!(0 <= northsouth && northsouth < NORTHSOUTH))return 1;
	...
	namespace ADCN
	{
		int FindGlobalBin(int& bin, int layer, int ladder, int northsouth, int chip, int channel, int adc)
		{
			...
			bin = adc + ADC * (channel + CHANNEL * (chip + CHIP * (northsouth + NORTHSOUTH * (ladder))));
